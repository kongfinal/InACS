...
$_SESSION["IDTermFirstStudent"] = $dataTerm[count($dataTerm)-1][2];

for ($x = count($dataTerm)-1; $x >= 0; $x-=1) {
    $idTerm = $dataTerm[$x][2];
    $NumTerm = $dataTerm[$x][1];
    $YearTerm = $dataTerm[$x][0];
    if($_SESSION["IDTermStudent"] == $idTerm){
...
addCoursetoTable($_SESSION["IDTermStudent"]);
...
function addCoursetoTable($IdTermSearch){
    ...
    if(mysqli_num_rows($CourseTable) > 0){
    for ($x = 0; $x < 5; $x+=1) {
        $page = ($_SESSION["PaginationSelectStudent"]-1) * 5;
        ...
        $tableCourse  = $tableCourse."<tr ondblclick=document.location.href='FManagementStudent.php?idCourse=$rowID' title=ดับเบิลคลิกเพื่อไปหน้าจัดการนิสิตของรายวิชานี้>";
    ...
    return $_SESSION["CourseTableInStudent"] = $tableCourse;
}

function CreatePagination($CourseNum){

    $lastPage = 0;
    if($_SESSION["PaginationSelectStudent"] == 1){
    ...
    for ($x = 0; $x*5 < $CourseNum;) {
        $x+=1;
        if($_SESSION["PaginationSelectStudent"] == $x){
    ...
    if($_SESSION["PaginationSelectStudent"] == $lastPage || ($_SESSION["PaginationSelectStudent"] == 1 && $lastPage == 0)){
    ...
    return $_SESSION["PaginationCourseTableInStudent"] = $Pagination;
}
                ...
                <div class="columns">
                    <div class="column is-8">
                        ...
                        <div class="set-flex" >
                            ...
                            <form name="changeTerm" action="FManagementStudent.php" method="post" style="margin-bottom: 0%;">
                        ...
                        <table id="table-starter">
                            ...
                            <?php
                                echo $_SESSION["CourseTableInStudent"] ;
                            ?>
                            <!--<tr ondblclick="document.location.href='student-management.php'">
                                <td>88624359</td>
                        ...
                        <div class="pagination" style="width: 100%; text-align: center;">
                            ...
                            <form name="changePagination" action="FManagementStudent.php" method="post" style="margin-bottom: 0%;">
                                <?php
                                    echo $_SESSION["PaginationCourseTableInStudent"] ;
                                ?>
                            </form>
                            </div>
                        </div>
